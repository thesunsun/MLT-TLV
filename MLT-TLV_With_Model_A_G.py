...
SETTINGS = {
    "data_dir": ".",
    "output_dir": "Your-MTL-TLV-With-Model-A-G-Output",
...
class TeeStream:
    """Mirror console output to both the terminal and a UTF-8 log file."""
...
def print_experiment_header(device: torch.device, output_root: Path) -> None:
    """Write reproducibility metadata at the beginning of the complete training log."""
    print('=' * 100)
    print('MTL-TLV A-H COMPLETE TRAINING LOG')
    print('=' * 100)
    print(f"Run start time       : {datetime.now().astimezone().isoformat(timespec='seconds')}")
...
def plot_confusion_matrix(true_labels: np.ndarray, pred_labels: np.ndarray, output_path: Path) -> None:
    ...
    image = ax.imshow(cm, interpolation='nearest', cmap='gray')
...
def train_and_save_model(model_id: str, raw_data: np.ndarray, fft_data: np.ndarray, labels: np.ndarray,
    ...
    best_acc_text = f"{best_metrics['accuracy'] * 100:.2f}%" if math.isfinite(best_metrics['accuracy']) else 'N/A'
    best_precision_text = f"{best_metrics['precision'] * 100:.2f}%" if math.isfinite(best_metrics['precision']) else 'N/A'
    best_recall_text = f"{best_metrics['recall'] * 100:.2f}%" if math.isfinite(best_metrics['recall']) else 'N/A'
    best_mae_text = f"{best_metrics['mae']:.4f}" if math.isfinite(best_metrics['mae']) else 'N/A'
    print('-' * 88)
    print(f'Model {model_id} best checkpoint')
    print(f'  Accuracy             : {best_acc_text}')
    print(f'  Precision            : {best_precision_text}')
    print(f'  Recall               : {best_recall_text}')
    print(f'  MAE                  : {best_mae_text}')
...
def print_ablation_summary(results: List[Dict[str, object]]) -> None:
    ...
    for result in results:
        ...
        precision = float(result.get('precision', float('nan')))
        recall = float(result.get('recall', float('nan')))
        accuracy_text = (
            f"{accuracy * 100:.2f}"
...
def main() -> None:
    ...
    # One complete log for the entire run: SMAC + Models A-H + final summary.
    logs_dir = (script_dir / 'Your-MTL-TLV-With-Model-A-G-Output').resolve()
    logs_dir.mkdir(parents=True, exist_ok=True)
    log_path = logs_dir / 'Your_ablation_A_H_complete_training.log'
    ...
    with log_path.open('w', encoding='utf-8', buffering=1) as log_file:
        ...
        try:
            ...
            set_reproducibility(SETTINGS['seed'])
            print_experiment_header(device, output_root)
            ...
            print('\n' + '=' * 100)
            print(f"Run completed successfully: {datetime.now().astimezone().isoformat(timespec='seconds')}")
            print(f'Complete training log: {log_path.name}')
            print('=' * 100)
        except Exception:
            print('\n' + '!' * 100, file=sys.stderr)
            print(f"Run failed: {datetime.now().astimezone().isoformat(timespec='seconds')}", file=sys.stderr)
            logging.exception('Unhandled exception during the A-H experiment')
